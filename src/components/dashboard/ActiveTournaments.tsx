  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-semibold text-gray-900 dark:text-white text-lg">{t.dashboard.tournaments}</h2>
          ...
          <Link key={tournament.id} href={`/tournaments/${tournament.id}`}>
            <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{tournament.name}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{tournament.registeredTeams.length}/{tournament.maxTeams} teams · {tournament.startDate}</p>
              </div>
              <Badge variant={statusVariant[tournament.status]}>
                {tournament.status === "active" ? t.tournaments.active
                  : tournament.status === "registration" ? t.tournaments.registration
                  : tournament.status === "completed" ? t.tournaments.completed
                  : tournament.status}
              </Badge>
            </div>
          </Link>
